...
# We'll build a domain-locked/minified version of the user's new script.
# Safer "minifier": remove /* */ comments; remove // only when it starts the line (ignores URLs in strings);
# collapse repeated whitespace outside of string literals is tricky; we'll just trim leading/trailing spaces from lines
# and join, keeping template literals intact. We'll also keep general spaces to avoid breaking HTML attributes.
original_code = r"""// --- CONFIGURACIÓN INICIAL ---
...
code = re.sub(r"/\*[\s\S]*?\*/", "", original_code)

# Remove line comments only if they start the line (ignoring leading spaces)
code_lines = []
...
# Domain guard (prepend, no wrap)
guard = "(function(h){h=(h||'').toLowerCase();if(h!=='pokemon-alparius.foroactivo.com'){console.warn('[pk-combate] bloqueado por dominio:',h);throw new Error('domain-locked');}})(location.hostname);\n"
wrapped = guard + code  # << en vez de envolver, lo agregamos arriba


# Save to file
out_path = "/mnt/data/combate-domainlocked.min.js"
with open(out_path, "w", encoding="utf-8") as f:
    f.write(wrapped)

# Compute SRI (sha384)
sha384 = hashlib.sha384(wrapped.encode("utf-8")).digest()
sri = "sha384-" + base64.b64encode(sha384).decode("ascii")
