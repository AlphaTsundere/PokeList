...
(function(){
  ...
  const BTN_ID   = 'ft-toggle';
  const PANEL_ID = 'ft-panel-v2';
  ...
  // CSS / UI
  const CSS = `
    #${BTN_ID} { position: fixed; left: 14px; bottom: 14px; z-index: 9999;
      padding: 8px 10px; border-radius: 10px; border: 1px solid #00000033; cursor: pointer;
      font: 600 13px system-ui; box-shadow: 0 6px 24px rgba(0,0,0,.35); background:#1d1d1d; color:#fff; }
    #${PANEL_ID} { position: fixed; left: 14px; bottom: 56px; z-index: 9999; width: 320px; max-height: 65vh;
      overflow:auto; padding: 10px; background: #121212; color: #eee; border: 1px solid #00000044;
    ...
    .ftn-btnth-small { padding:4px 8px; font: 600 12px system-ui; border-radius:8px; border:1px solid #00000033; background:#1d1d1d; color:#fff; cursor:pointer; }
  ...
  function ensureCss(){
    if (!document.getElementById('ft-css')){
      ...
      st.id='ft-css'; st.textContent = CSS;
  ...
  function hex6ToHex8(hex6, a){ return hex6 + hex(Math.round(clamp01(a)*255)); }
  ...
  function makeRow(vname){
    ...
    const aRange = document.createElement('input'); aRange.type='range'; aRange.min='0'; aRange.max='100';
    aRange.value = String(Math.round(a*100)); aRange.style.flex='0 0 90px';
  ...
  // Export / Import
  function snapshotVars(){ const kv={}; [...VARS_LIGHT,...VARS_DARK].forEach(v=>{ kv[v]=getVar(v)||''; }); return kv; }
  function applyVarsObject(obj){
    if (!obj || typeof obj!=='object') return;
    Object.entries(obj).forEach(([k,val])=>{ if (typeof val === 'string') setVar(k,val); });
    const lightVals = VARS_LIGHT.map(v=> getVar(v)||''); const darkVals = VARS_DARK.map(v=> getVar(v)||'');
    save(LS_LIGHT, lightVals); save(LS_DARK, darkVals);
  }
  ...
  function buildPanel(){
    ...
    panel.innerHTML = `
      <div class="ft-head">
        ...
        <div class="ft-actions">
          <button class="ftn-btnth-small js-export" title="Exportar colores">Exportar</button>
          <button class="ftn-btnth-small js-import" title="Importar colores">Importar</button>
          <input type="file" accept="application/json" class="ft-file" style="display:none" />
          <button class="ftn-btnth-small js-reset-light" title="Reset light">Reset light</button>
          <button class="ftn-btnth-small js-reset-dark" title="Reset dark">Reset dark</button>
        </div>
      </div>

      <details class="ft-acc js-acc-light">
        <summary>🌞 Cambiar colores light</summary>
        <div class="ft-body js-light"></div>
      </details>

      <details class="ft-acc js-acc-dark">
        <summary>🌙 Cambiar colores dark</summary>
    ...
    // Abrir el acordeón del tema actual
    syncAccordionByTheme(panel);
    return panel;
  }

  // Elimina botones/paneles antiguos que puedan chocar
  function killOldUI(){
    document.querySelectorAll('.ft-btn, .theme-toggle').forEach(el => el.remove()); // botones viejos
    // paneles viejos (con misma clase o sin ID nuevo)
    document.querySelectorAll('.ft-panel, [id^="ft-panel"]').forEach(el => {
  ...
  function mount(){
    ...
    if (!btn){
      ...
      btn.id = BTN_ID;
      btn.textContent = 'Colores'; // <— cambia el texto aquí
